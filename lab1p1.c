...
int main(void)
{
	...
	while(1)
	{
		...
		// LED is illuminated (on).
                ...
                {
                    case 0:
                        if(PORTBbits.RB2 == 0x0) //if button is pressed, switch state
                            debounceDelay();
                        else
                            LATAbits.LATA1 = 0x0; //turn Right LED on
                    break;
                    case 1:
                        if(PORTBbits.RB2 == 0x1) //if button is not pressed, switch state
                            debounceDelay();
                        else
                            LATAbits.LATA0 = 0x0; //turn Left LED on
                    break;
                }
            }
...
void __attribute__((interrupt,auto_psv)) _T1Interrupt(void)
{
    ...
    if(state == 0)
    {
        LATAbits.LATA1 = 0x1; //turn Left LED off
        state = 1;
    }
    else
    {
        LATAbits.LATA0 = 0x1; //turn Right LED off
        state = 0;
    }
}
